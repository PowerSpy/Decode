...
//gate intake and shoot
        ...
        Globals.AUTO_ENDING_POSE = ROBOT_POSITION.clone();
        robot.waitWhile(() -> {
            Globals.AUTO_ENDING_POSE = ROBOT_POSITION.clone();
            return true;
        });
    ...
    private void shoot(double heading, int shotType, boolean move, boolean first) {
        Path shootPath = new Path(new Pose2d(ROBOT_POSITION.x, ROBOT_POSITION.y, ROBOT_POSITION.heading))
                .addPoint(new Pose2d(-18, 18, heading), false, false);

        Path firstShot = new Path(new Pose2d(ROBOT_POSITION.x, ROBOT_POSITION.y, ROBOT_POSITION.heading))
                .addPoint(new Pose2d(-36, 36, heading), false, false);
        robot.shooter.reqAim(true);
        if (move) {
            if (first) {
                robot.drivetrain.setPath(firstShot);
                ...
                robot.drivetrain.setPath(shootPath);
        ...
        robot.shooter.reqStop(true);
        /*
        not necessary cs gvf should do the turning while moving
        if (first) {
    ...
    private void intake(double x, double y, boolean skipLast) {
        Path intakeBalls = new Path(new Pose2d(ROBOT_POSITION.x, ROBOT_POSITION.y, ROBOT_POSITION.heading))
                .addPoint(new Pose2d(x, 22, Math.PI/2), false, false)
                .addPoint(new Pose2d(x, y, Math.PI/2), false, false);
        ...
        */
        robot.drivetrain.setPath(intakeBalls);
    ...
    private void gate_intake(boolean skipLast) {
        ...
        */
        ...
        robot.drivetrain.setPath(gate_intake1);
        ...
        */
        ...
        robot.drivetrain.setPath(gate_intake2);
